...
void send_msg_to_client(CLIENT_INFO *client_info, const char *msg, int len);
...
int main(int argc, char *argv[])
{
                ...
                CLIENT_INFO client_info[MAX_CLNT] = {{0,-1,"","LSW_SQL","PASSWD"}, \
                                {0,-1,"","LSW_UBT","PASSWD"},  {0,-1,"","LSW_ARD","PASSWD"}, \
                                {0,-1,"","LSW_AND","PASSWD"},  {0,-1,"","LSW_LIN","PASSWD"}, \
                                {0,-1,"","LSW_BT","PASSWD"},  {0,-1,"","LSW_TEST","PASSWD"}, \
                                {0,-1,"","LSW_JET","PASSWD"},  {0,-1,"","9","PASSWD"}, \
...
void *clnt_connection(void *arg)
{
    ...
    while (1) {
        ...
        // LSW_ARD에서 받은 메시지는 모든 클라이언트에게 전송
...
void send_msg(MSG_INFO * msg_info, CLIENT_INFO * first_client_info)
{
    ...
    if (!strcmp(msg_info->from, "LSW_ARD")) {
        // LYH_ARD에서 수신한 메시지를 모든 클라이언트에게 전송
        for (i = 0; i < MAX_CLNT; i++) {
